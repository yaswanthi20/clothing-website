...
if (process.env.RAZORPAY_KEY_ID && 
    ...
    console.log('⚠ Razorpay keys not configured - use /api/payment-mock endpoints for testing');
}
...
router.post('/create-order', requireAuth, async (req, res) => {
    ...
    if (!razorpay) {
        return res.status(503).json({ 
            error: 'Razorpay not configured. Please use /api/payment-mock/create-order for testing or configure Razorpay keys.',
            useMock: true
        });
    }
...
router.post('/verify-payment', requireAuth, async (req, res) => {
    ...
    if (!razorpay) {
        return res.status(503).json({ 
            error: 'Razorpay not configured. Please use /api/payment-mock/verify-payment for testing.',
            useMock: true
        });
    }
